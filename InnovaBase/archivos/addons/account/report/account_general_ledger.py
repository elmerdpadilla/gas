...
class general_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update( {
            'time': time,
            'currency': self.lines3,
            'lines': self.lines,
            'sum_debit_account': self._sum_debit_account,
            'sum_credit_account': self._sum_credit_account,
            'sum_credit_account_currency': self._sum_credit_account_currency,
            'sum_debit_account_currency': self._sum_debit_account_currency,
            'sum_balance_account': self._sum_balance_account,
            'sum_balance_account_currency': self._sum_balance_account_currency,
    ...
    def lines3(self, account):
        ...
        res = res_lines
        for l in res:
            return self.pool.get('res.currency').browse(self.cr,self.uid, l['currency_id'],context=None)


    def lines2(self, account):
        """ Return all the account_move_line of account with their account code counterparts """
        move_state = ['draft','posted']
        if self.target_move == 'posted':
            move_state = ['posted', '']
        ...
        for l in res:
            ...
            if l['amount_currency'] != None:
                ...
                l['progress']=self.tot_currency
            l['currency_id']=self.pool.get('res.currency').browse(self.cr,self.uid, l['currency_id'],context=None)
        return res
    ...
    def _sum_credit_account(self, account):
        ...
        if self.init_balance:
            ...
            sum_credit += self.cr.fetchone()[0] or 0.0
        return sum_credit
    def _sum_credit_account_currency(self, account):
        if account.type == 'view':
            return account.credit
        move_state = ['draft','posted']
        if self.target_move == 'posted':
    ...
    def _sum_debit_account_currency(self, account):
        if account.type == 'view':
            return account.debit
        move_state = ['draft','posted']
        if self.target_move == 'posted':
    ...
    def _sum_balance_account(self, account):
        ...
        if self.init_balance:
            ...
            sum_balance += self.cr.fetchone()[0] or 0.0
        return sum_balance
    def _sum_balance_account_currency(self, account):
        if account.type == 'view':
            return account.balance
        move_state = ['draft','posted']
        if self.target_move == 'posted':
            move_state = ['posted','']
        self.cr.execute('SELECT (sum(amount_currency)) as tot_balance \
                FROM account_move_line l \
                JOIN account_move am ON (am.id = l.move_id) \
                WHERE (l.account_id = %s) \
                AND (am.state IN %s) \
