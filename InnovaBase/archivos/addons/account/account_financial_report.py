...
class account_financial_report(osv.osv):
    ...
    def _get_balance(self, cr, uid, ids, field_names, args, context=None):
        ...
        for report in self.browse(cr, uid, ids, context=context):                        
            ...
            if report.type == 'accounts':
                ...
                for key, value in res2.items():
                    for field in field_names:
                        if not financial_obj.browse(cr,uid,key,context=context).type=='account_report':
                        #if not report.type='':
                            res[report.id][field] += value[field]
    ...
    _columns = {
        'underline':fields.boolean('Mostrar linea de total'),
        'mostrar_saldo':fields.boolean('Mostrar Saldo'),
