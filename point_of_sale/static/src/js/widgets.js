function openerp_pos_widgets(instance, module){ //module is instance.point_of_sale
	...
	this.pos.bind('change:selectedOrder', this.renderElement, this);
        ...
        renderElement: function() {
            var self = this;
            this._super();
            _.each(this.pos.discounts,function(discount) {
                var button = new module.discountButtonWidget(self,{
                    pos: self.pos,
                    pos_widget : self.pos_widget,
                    discount: discount,
                });
    ...
    module.discountButtonWidget = module.PosBaseWidget.extend({
        template: 'discountButtonWidget',
        init: function(parent, options){
            this._super(parent, options);
		...
		if (order.getSelectedLine()){
			if(self.discount.total){
				if(self.discount.discount!=0){
					if (order.getSelectedLine().get_discount()!=self.discount.discount) {
						order.get('orderLines').each(function(line){
							line.set_discount(self.discount.discount);
							line.set_discount2(self.discount.name);
            						});
					...
					var discount=prompt("Please enter your Discount", "0");
						order.get('orderLines').each(function(line){
				...
				if(self.discount.discount!=0){
					if (order.getSelectedLine().get_discount()!=self.discount.discount) {
						order.getSelectedLine().set_discount( self.discount.discount);
						order.getSelectedLine().set_discount2( self.discount.name);
					}else{
						order.getSelectedLine().set_discount( 0);
						order.getSelectedLine().set_discount2( "");
					}
				}
				else{
					var discount=prompt("Please enter your Discount", "0");
						order.getSelectedLine().set_discount( discount);
						order.getSelectedLine().set_discount2( self.discount.name);
				}
			}
		}
    ...
    module.OrderWidget = module.PosBaseWidget.extend({
        ...
        update_summary: function(){
            var order = this.pos.get('selectedOrder');
            var subtotal     = order ? order.getTotalTaxExcluded() : 0; //hluna 4/11/2014 total sin impuesto
            var taxes     = order ? order.getTotalTaxIncluded() - order.getTotalTaxExcluded() : 0; //hluna 4/11/2014 impuesto 
	    var total     = order ? order.getTotalTaxIncluded() : 0; //hluna 4/11/2014 total sin impuesto
	    var discount = order ? order.getDiscountTotal() :0;
	    this.el.querySelector('.summary .total > .value2').textContent = this.format_currency(subtotal);
            this.el.querySelector('.summary .total > .value').textContent = this.format_currency(total);
            this.el.querySelector('.summary .total > .value3').textContent = this.format_currency(taxes);
	   // this.el.querySelector('.summary .total > .value4').textContent = this.format_currency(discount);
...
// ---------- Main Point of Sale Widget ----------
	...
	crear_buton: function() {
	    this.discount = new module.discountWidget(this, {});
            this.discount.replace(this.$('.placeholder-discountWidget'));
	},
