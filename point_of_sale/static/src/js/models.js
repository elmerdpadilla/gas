function openerp_pos_models(instance, module){ //module is instance.point_of_sale
    ...
    module.PosModel = Backbone.Model.extend({
        ...
        models: [
        {
            ...
            model: 'ir.sequence',
            fields: [],
            domain: function(self){ return [['id','=', self.config.sequence_id[0]]]; },
            loaded: function(self, sequence){ self.sequence=sequence[0];self.pos_session.sequence_number=self.sequence.number_next_actual;},
        },{
            ...
            model:  'pos.innova.discount',
            fields: ['discount','name','total'],
            domain:  function(self){ return [['pos_id','=',self.pos_session.config_id[0]]]; },
...
//setInterval
    ...
    module.Orderline = Backbone.Model.extend({
	    ...
	    orderline.discount2 = this.discount2;
	...
	set_discount2: function(discount){
    ...
    module.Order = Backbone.Model.extend({
	    ...
	    var retorno;
		if(this.pos.sequence.prefix){
			retorno=this.pos.sequence.prefix +zero_pad(this.sequence_number,this.pos.sequence.padding);
			}else{
			retorno=zero_pad(this.sequence_number,this.pos.sequence.padding);
}
            return retorno;
        },
               generateUniqueId3: function() {
		 self=this;
            function zero_pad(num,size){
                var s = ""+num;
                while (s.length < size) {
            ...
            return this.pos.sequence.prefix +
                   zero_pad(this.cpsn,this.pos.sequence.padding);
        },
	cleartime: function() {
		clearTimeout(this.nodetime);
        },

	getTypecash2: function() {
            return (this.get('paymentLines')).reduce((function(sum, paymentLine) {
                return (sum ||( paymentLine.cashregister.journal.type=='cash'));
            }), false);
        },
        generateUniqueId4: function() {
		var self=this;
		var p;
		var s=new openerp.web.Model('ir.sequence').query(['prefix','number_next_actual'])
		.filter([['id','=',this.pos.config.sequence_id[0]]])
		.first();
		

		p=s.then(function(a){	
			self.sequence=a.number_next_actual;
			self.sequence=a.number_next_actual;
			self.a=a.number_next_actual;
			return a;
		});;
