function openerp_pos_db(instance, module){ 
 ...
 var N = products.length;
        var incremento = parseInt(N);
        do {
            incremento = parseInt(incremento / 2);
            for (var k = 0; k < incremento; k++) {
                for (var i = incremento + k; i < N; i += incremento) {
                    var j = i;
                    while (j - incremento >= 0 && products[j].price < products[j - incremento].price) {
                        var tmp = products[j];
                        products[j] = products[j - incremento];
                        products[j - incremento] = tmp;
                        j -= incremento;
                    }
                }
            }
 ...
 var N = list.length;
        ...
        do {
            ...
            for (var k = 0; k < incremento; k++) {
                for (var i = incremento + k; i < N; i += incremento) {
                    var j = i;
                    while (j - incremento >= 0 && list[j].price < list[j - incremento].price) {
                        var tmp = list[j];
                        list[j] = list[j - incremento];
                        list[j - incremento] = tmp;
                        j -= incremento;
                    }
                }
            }
